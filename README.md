...
The solution should return an object (see below) for each month between the windowStartDate and windowEndDate inclusive. Vacancy is determined based off whether the rentDueDate is before or after the leaseStartDate.

**Proration:**

If a lease does not begin on the agreed upon monthly due date, then the first month's rent will be prorated for the remaining days until the next due date (based on a 30 day month). i.e. if there are 31 or 28 days, for the purpose of calculating the pro-rated rent you would assume there are 30 days in the month for your denominator.
...
{
    `vacancy`: boolean, // is the unit vacant? If rentDueDate < leasStartDate vacancy should == true, otherwise vacancy should == false
...
**You should expect the below output:**
```
[
    {
        vacancy: true,
